        ...
        try:
            ...
            if not project_id:
                return self._create_response(
                    400, {"error": "Missing project_id in the message"}
                )

            if not user_id:
                return self._create_response(
                    400, {"error": "Missing user_id in the message"}
                )

            if action != "process":
                return self._create_response(
                    400, {"error": f"Unsupported action: {action}"}
                )

            # Get project details to verify the project exists
            project_files_data = self.db_manager.get_files_info(project_id)

            if not project_files_data:
                return self._create_response(
                    404, {"error": f"Project with ID {project_id} not found"}
                )

            # Convert MongoDB document to FileInfo object
            file_infos = []
            for file in project_files_data:
                file_info = FileInfo.from_dict(file)
                file_infos.append(file_info)

            # Process the files
            processing_result = self.file_service.process_files(file_infos)
            ...
            if processing_result:
                return self._create_response(
                    ...
                    {
                        "status": "success",
                        "message": f"Successfully processed {len(file_infos)} files for project {project_id}",
